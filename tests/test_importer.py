...
def load_json_fixture_file(file_name):
    json_file = "{fixtures_dir}/{file_name}".format(
        fixtures_dir=FIXTURES_DIR, file_name=file_name
    )
...
class TestGetLoxoneStructureFile(object):
    ...
    def test_internal_server_error(self, mock_get, configured_importer):
        mock_resp = mock_requests_response(
            status=requests.codes.internal_server_error,
            raise_for_status=HTTPError("ERROR"),
        )
        mock_get.return_value = mock_resp
    ...
    def test_timeout(self, mock_get, configured_importer):
        mock_resp = mock_requests_response(
            status=None, raise_for_status=Timeout("TIMEOUT")
        )
        mock_get.return_value = mock_resp
...
@pytest.mark.parametrize(
    "loxone_structure_file,loxone_controls",
    [
        (
            '{ "controls": { "99999999-9999-9999-9999999999999999": {"name": "test"} } }',
            '{ "99999999-9999-9999-9999999999999999": {"name": "test"} }',
        ),
        ('{ "controls": { } }', "{}"),
    ],
)
@pytest.mark.usefixtures("unconfigured_importer")
def test_get_loxone_controls(
    loxone_structure_file, loxone_controls, unconfigured_importer
):
    expected = json.loads(loxone_controls)
    actual = unconfigured_importer.get_loxone_controls(
        json.loads(loxone_structure_file)
    )
    assert actual == expected


@pytest.mark.parametrize(
    "loxone_structure_file,loxone_rooms",
    [
        (
            '{ "rooms": { "99999999-9999-9999-9999999999999999": {"name": "test"} } }',
            '{ "99999999-9999-9999-9999999999999999": {"name": "test"}, "00000000-0000-0000-0000000000000000": {"uuid": "00000000-0000-0000-0000000000000000", "name": ""} }',
        ),
        (
            '{ "rooms": { } }',
            '{ "00000000-0000-0000-0000000000000000": {"uuid": "00000000-0000-0000-0000000000000000", "name": ""} }',
        ),
    ],
)
@pytest.mark.usefixtures("unconfigured_importer")
...
@pytest.mark.parametrize(
    "loxone_structure_file,loxone_categories",
    [
        (
            '{ "cats": { "99999999-9999-9999-9999999999999999": {"name": "test"} } }',
            '{ "99999999-9999-9999-9999999999999999": {"name": "test"}, "00000000-0000-0000-0000000000000000": {"uuid": "00000000-0000-0000-0000000000000000", "name": "undefined", "type": "undefined"} }',
        ),
        (
            '{ "cats": { } }',
            '{ "00000000-0000-0000-0000000000000000": {"uuid": "00000000-0000-0000-0000000000000000", "name": "undefined", "type": "undefined"} }',
        ),
    ],
)
@pytest.mark.usefixtures("unconfigured_importer")
def test_get_loxone_categories(
    loxone_structure_file, loxone_categories, unconfigured_importer
):
    expected = json.loads(loxone_categories)
    actual = unconfigured_importer.get_loxone_categories(
        json.loads(loxone_structure_file)
    )
    assert actual == expected
...
class TestGenerateHaBridgeDevicesConfiguration(object):
    ...
    @pytest.mark.parametrize(
        "loxone_structure_file,ha_bridge_devices_configuration",
        [
            ("LoxAPP3_1.json", "LoxAPP3_1_ha_bridge.json"),
            ("LoxAPP3_2.json", "LoxAPP3_2_ha_bridge.json"),
        ],
    )
    def test_generate_ha_bridge_devices_configuration(
        self,
        loxone_structure_file,
        ha_bridge_devices_configuration,
        configured_importer,
    ):
        actual = configured_importer.generate_ha_bridge_devices_configuration(
            load_json_fixture_file(loxone_structure_file)
        )
...
class TestAddDevicesIntoHaBridge(object):
    ...
    def test_ok(self, mock_post, configured_importer):
        ...
        mock_resp = mock_requests_response(
            status=requests.codes.created, json_data='{"key": "value"}'
        )
    ...
    def test_internal_server_error(self, mock_post, configured_importer):
        ...
        mock_post.return_value = mock_resp
        with pytest.raises(HTTPError):
            configured_importer.add_devices_into_ha_bridge(
                ha_bridge_devices_configuration
            )
    ...
    def test_with_mandatory_parameter(self, mock_importer, cli_runner):
        actual = cli_runner.invoke(
            ...
            [
                ...
                "--loxone-username=player1",
                "--loxone-password=secret",
            ],
        )
        assert actual.exit_code == 0
        assert (
            "Retrieve visualisation structure file from Loxone MiniServer"
            in actual.output
        )
        assert (
            "Generate HA-Bridge devices configruation from visualisation structure file"
            in actual.output
        )
